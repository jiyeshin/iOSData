...
class MovieListController: UITableViewController {
    ...
    @objc func handleRequest(_ refreshControl:UIRefreshControl){
        ...
        //refreshConrol 애니메이션 중지
        refreshControl.endRefreshing()
    }
    
    //파싱한 결과를 저장할 list 변수 - 지연생성을 이용
    //지연생성: 처음부터 만들어두지 않고 처음 사용할 떄 생성
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        //비동기적으로 이미지 출력
        DispatchQueue.main.async(execute: {cell.thumbnailImage.image = self.getThumnailImage(indexPath.row)
        })
        return cell
    }
